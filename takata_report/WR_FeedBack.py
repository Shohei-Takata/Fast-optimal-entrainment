...
for tt in range(Tsimu_num):
    ...
    if(tt%PhiCount==0):
        X_phase2 = Pha.Calc_phase_directory(x, WR.dif, X0_, Tnum, dt, rotations = 5) # measure phase [0, Tnum-1]
        # With the appropriate α, |X_phase2 - X_phase| < 1e-3*Tnum. So X_phase is sufficiently accurate. 
        if(X_phase2!=X_phase):
            print("t = ", tt*dt, " diff = ", X_phase2-X_phase)

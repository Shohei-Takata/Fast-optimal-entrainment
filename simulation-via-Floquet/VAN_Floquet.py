...
tmax = 1000 # running time for convergence
dt = 2.5e-4 # step width

ND = my.ND(tmax, dt)

# Van der Pol
#di/dt = self.a*i - i**3/3.0 - v + self.x0
#dv/dt = i + self.y0
a = 0.5
VAN = my.VAN(a, x0 = 0.0, y0 = 0.0)
...
# run to convergence
Xstart = ND.evol_to_convergence(VAN.dif, Xstart)
print("converged X = ", Xstart.T)

# search period

# The phase is 0 when the y falls below y_basis = 0.0
Tnum, Xstart = ND.find_Tnum_Y(VAN.dif, Xstart, 0.0)
T = dt * Tnum
omega = 2*pi/T
...
Floquet = my.Floquet(VAN.dif, VAN.Jacobian, Tnum, T, omega, dt)

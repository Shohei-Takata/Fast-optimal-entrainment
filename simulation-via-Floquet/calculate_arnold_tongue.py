...
def simple_Average(P, Delta):
    ...
    if mu == 0 and nu == 0:
        PhaseAverage = np.array([1e4]) # not synchronize
        return PhaseAverage
    ...
    if AP != Division:
        print("Error P = ", P, " Delta = ", Delta, " tA = ", AP)
    return np.array([PhaseAverage])
...
def feedback_Average(P, Delta, alpha):
    ...
    for tt in range(Tsimu_num):
        ...
        if tt >= SA:
            ti += 1
            if(ti%PhiCount==0):
                X_phase = Pha.Calc_phase_directory(x, VAN.dif, X0_, Tnum, dt) # measure phase [0, Tnum-1]
                Phi = Pha.Trans_PI(X_phase - EFP, Tnum) # Φ(t) [-π, π]
                PhaseAverage += Phi
...
for n in range(P_number):
    P = 1e-4*(100**(1/(P_number-1)))**n
    for m in range(D_number):
        Delta = 1e-3 * (m - D_number//2) * (10**(1/(P_number-1)))**n
